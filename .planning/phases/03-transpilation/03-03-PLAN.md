...
must_haves:
  truths:
    - "CLI backs up existing OpenCode configs before overwrite"
...
1. **Backup existing files**:
   - Create timestamped directory: `.opencode-backup/YYYY-MM-DD_HHMMSS/`
...
2. **Detect OpenCode config directory**:
   - Check `~/.config/opencode` (Linux/Mac standard)
   - Check `%APPDATA%\opencode` (Windows standard)
   - Check project root `.opencode/` (project-local)
...
- Orchestrator detects OpenCode config directory (tries multiple locations)
...
- Idempotency check skips re-run if source unchanged (unless --force)
  ...
  <done>
Idempotency checker implemented with content hashing. Orchestrator implements full pipeline with backup/rollback. Dry-run validates without writing. OpenCode directory auto-detection works.
  </done>
</task>
...
<verification>
After all tasks:
- BackupManager creates timestamped backups with manifest
...
<success_criteria>
1. Transpilation backs up existing configs before overwrite
