...
2. **Check idempotency**: Compare current hash with last run in `.gsdo-manifest.json`
...
export async function writeManifest(manifest: GSDOManifest, manifestPath: string): Promise<void>;
```
...
9. **On success**: Write `.gsdo-manifest.json`, log backup location
...
Add `TranspileOptions`, `TranspileResult`, `GSDOManifest` to src/types/index.ts.
