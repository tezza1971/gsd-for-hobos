...
must_haves:
  truths:
    ...
    - "Transform rules can be overridden by user via ~/.gsdo/transforms.json"
  artifacts:
...
This file ships with the package. Users can override by creating `~/.gsdo/transforms.json` with same structure.
...
1. Load transform rules:
   - Try `~/.gsdo/transforms.json` first (user override)
...
Use node:fs/promises to read transform rules. Use node:path to resolve `~/.gsdo/transforms.json`.
...
- User override at `~/.gsdo/transforms.json` is loaded if exists
...
3. **User overrides**: Mock `~/.gsdo/transforms.json`, verify user rules override defaults
...
- User can override rules via `~/.gsdo/transforms.json`
...
2. User overrides work via `~/.gsdo/transforms.json`
